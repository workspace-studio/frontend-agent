...
import HeadingBlock from '@/components/TextEditor/components/HeadingBlock';
import ImageBlock from '@/components/TextEditor/components/ImageBlock';
import LinkBlock from '@/components/TextEditor/components/LinkBlock';
...
const EditorToolbar = ({
  ...
  const isEnabled = (feature: keyof NonNullable<typeof toolbarConfig>) => {
    if (!toolbarConfig) return true;

    return toolbarConfig[feature] !== false;
  };

  const shouldShowDivider = (nextSectionEnabled: boolean) => nextSectionEnabled && !toolbarConfig?.customButtons;
  ...
  const handleSetAlignCenter = () => editor.chain().focus().setTextAlign('center').run();
  const handleSetAlignRight = () => editor.chain().focus().setTextAlign('right').run();
  const handleSetAlignJustify = () => editor.chain().focus().setTextAlign('justify').run();

  const handleToggleCode = () => editor.chain().focus().toggleCode().run();
  const handleToggleCodeBlock = () => editor.chain().focus().toggleCodeBlock().run();
  ...
  const handleSetHardBreak = () => editor.chain().focus().setHardBreak().run();
  const handleClearFormatting = () => editor.chain().focus().clearNodes().unsetAllMarks().run();

  const handleUndo = () => editor.chain().focus().undo().run();
  const handleRedo = () => editor.chain().focus().redo().run();

  return (
    <Stack direction="row" alignItems="center" flexWrap="wrap" className={styles.toolbarContainer}>
      {isEnabled('heading') && (
        <>
          <HeadingBlock
            editor={editor}
            levels={
              toolbarConfig?.heading && typeof toolbarConfig.heading === 'object'
                ? toolbarConfig.heading.levels
                : undefined
            }
          />
          {shouldShowDivider(isEnabled('textFormatting')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}

      {isEnabled('textFormatting') && (
        <>
          ...
          {shouldShowDivider(isEnabled('lists')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}
      ...
      {isEnabled('lists') && (
        <>
          ...
          {shouldShowDivider(isEnabled('alignment')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}
      ...
      {isEnabled('alignment') && (
        <>
          ...
          {shouldShowDivider(isEnabled('code') || isEnabled('blockquote') || isEnabled('horizontalRule')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}

      {(isEnabled('code') || isEnabled('blockquote') || isEnabled('horizontalRule')) && (
        <>
          <Stack direction="row" spacing={0.5}>
            {isEnabled('code') && (
              <>
            ...
            {isEnabled('horizontalRule') && (
              <ToolbarItem
                title={t('toolbar.horizontal-rule')}
          ...
          {shouldShowDivider(isEnabled('links') || isEnabled('images')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}

      {(isEnabled('links') || isEnabled('images')) && (
        <>
          <Stack direction="row" spacing={0.5}>
            {isEnabled('links') && <LinkBlock editor={editor} />}
            {isEnabled('images') && <ImageBlock editor={editor} />}
          </Stack>
          {shouldShowDivider(isEnabled('hardBreak') || isEnabled('clearFormatting')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}

      {(isEnabled('hardBreak') || isEnabled('clearFormatting')) && (
        <>
          <Stack direction="row" spacing={0.5}>
            {isEnabled('hardBreak') && (
              <ToolbarItem
                title={t('toolbar.hard-break')}
                aria-label="Hard break"
                onClick={handleSetHardBreak}
                icon={
            ...
            {isEnabled('clearFormatting') && (
              <ToolbarItem
                title={t('toolbar.clear-formatting')}
          ...
          {shouldShowDivider(isEnabled('undoRedo')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}
      ...
      {isEnabled('undoRedo') && (
        <>
          ...
          {shouldShowDivider(isEnabled('fullscreen')) && (
            <Divider orientation="vertical" flexItem className={styles.divider} />
          )}
        </>
      )}

      {isEnabled('fullscreen') && onToggleFullScreen && (
        <Stack direction="row" spacing={0.5}>
          <ToolbarItem
